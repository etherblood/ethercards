...
public class DisplayCardTemplateBuilder extends CardTemplateBuilder {
    ...
    public DisplayCardTemplate build(int id) {
        return new DisplayCardTemplate(id, !components.isEmpty(), manaCost, casts.stream().map(CardCastBuilder::build).toArray(CardCast[]::new), alias, name, flavourText, description, imagePath, colors, components, tribes, onCastEffects, onSummonEffects, onDeathEffects, onSurviveEffects, onUpkeepEffects, afterBattleEffects, componentModifiers);
    }
}
