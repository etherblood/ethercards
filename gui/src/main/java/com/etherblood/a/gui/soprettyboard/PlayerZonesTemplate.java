...
public class PlayerZonesTemplate {
    ...
    public PlayerZones create(Vector3f position, Quaternion rotation) {
        ...
        ScalingGridCardZone boardZone = new ScalingGridCardZone(position, rotation, battleBounds, cardSize);
        ScalingGridCardZone handZone = new ScalingGridCardZone(position.add(0, 1.2f, 0), rotation.mult(new Quaternion().fromAngles(FastMath.QUARTER_PI / 2, 0, 0)), handBounds, cardSize);
