...
import com.destrostudios.cardgui.samples.visualization.cards.modelled.LightingModelledCard;
...
public class CardVisualization extends CustomAttachmentVisualization<Node> {
    ...
    public CardVisualization(AssetManager assetManager) {
        node = new Node();
        modelledCard = new LightingModelledCard(assetManager, "models/card/card.j3o", "images/cardbacks/magic.png", ColorRGBA.Black);
        node.attachChild(modelledCard.getNode());
        glowBox = new GlowBox(assetManager, 0.96f, 1.28f);
    }
    private Node node;
    private LightingModelledCard modelledCard;
